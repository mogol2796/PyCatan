...
DEFAULT_MATCHES_PER_POSITION = 2
DEFAULT_POSITIONS = [0, 1, 2, 3]

DEFAULT_RESULTS_CSV = "benchmark_vs_llm_vs_random_resultados.csv"
DEFAULT_DETAILS_CSV = "benchmark_vs_llm_vs_random_detalle.csv"
...
PROMPT_PROFILES = {
    ...
    "economy_v1": {
        ...
        "build_system_prompt": (
            ...
            "For 'card', use {\"building\":\"card\"} (no node_id/road_to needed). "
            "If you cannot build anything, return {\"actions\":[{\"building\":\"none\"}]}. "
            "Economy focus: prefer building a town, then a city, then a road, then a card."
        ),
    },
    "cities_cards_v1": {
        "start_system_prompt": (
...
def _agent_label(agent_cls, prompt_tag=""):
...
def _run_match(agent_cls, position, max_rounds, store_trace, game_number):
...
def _parse_args():
    p = argparse.ArgumentParser(description="Experimentos ligeros: LLMJsonAgent vs 3 RandomAgent.")
...
if __name__ == "__main__":
    ...
    agent_name = _agent_label(agent_cls, prompt_tag=prompt_tag if agent_is_llm else "")
    ...
    total_games = max(1, int(args.matches)) * len(args.positions)
    print(f"\n== {agent_name} ==\nPartidas totales: {total_games} | posiciones={args.positions} | matches={args.matches}\n")
    ...
    for pos in args.positions:
        for m in range(int(args.matches)):
            ...
            status = "WIN" if r["victory"] else ("TIMEOUT" if r["timeout"] else ("DRAW" if r["draw"] else "LOSS"))
            winner_str = r["winner"] or "-"
            print(
                f"[{game_idx}/{total_games}] pos={pos} {status} "
